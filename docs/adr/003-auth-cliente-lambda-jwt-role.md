...
| **Kong + Konga (Gateway no Kubernetes)** | Kong seria outro API Gateway (com UI Konga). Já usamos **AWS API Gateway HTTP API** no repo da Lambda para `POST /auth/cpf` e proxy `/{proxy+}` → Nest. Introduzir Kong no EKS duplicaria a porta de entrada, somaria chart/ops e Konga só para admin — sem ganho neste recorte do lab. |
