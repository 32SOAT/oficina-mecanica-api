...
API Gateway HTTP API gerenciado pelo `oficina-mecanica-infra-k8s`:

- `POST /auth/cpf` → Lambda
- `ANY /{proxy+}` → NLB do Nest, usando o SSM
  `/oficina/<ambiente>/platform/api-nlb-hostname`, incluindo login admin e
  `/api/v1/estoque`
...
Detalhe de rotas: [auth.md](../architecture/auth.md). A decisão histórica de
separar os logins permanece; ownership do Gateway foi posteriormente
transferido para o `infra-k8s`.
...
| **Kong + Konga (Gateway no Kubernetes)** | Kong seria outro API Gateway (com UI Konga). Já usamos **AWS API Gateway HTTP API** no `infra-k8s` para `POST /auth/cpf` e proxy `/{proxy+}` → Nest. Introduzir Kong no EKS duplicaria a porta de entrada, somaria chart/ops e Konga só para admin — sem ganho neste recorte do lab. |
