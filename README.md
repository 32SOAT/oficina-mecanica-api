...
```bash
docker run --rm -t -v "${PWD}:/zap/wrk" zaproxy/zap-stable zap-api-scan.py -t http://host.docker.internal:3000/api-json -f openapi -r report.html -z "-config replacer.full_list(0).description=auth -config replacer.full_list(0).enabled=true -config replacer.full_list(0).matchtype=REQ_HEADER_STR -config replacer.full_list(0).matchstr=Authorization -config replacer.full_list(0).replacement='Bearer SEU_TOKEN'"
```
