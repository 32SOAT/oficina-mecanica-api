...
describe('OrdemServicoTypeormRepository', () => {
  ...
  describe('transições simples', () => {
    ...
    it('iniciarDiagnostico move Recebida → EmDiagnostico e emite eventos', async () => {
      ...
      await service.transicionar('os-1', S.EmDiagnostico);
    ...
    it('avancarStatus rejeita transição inválida', async () => {
      setupOs(S.Recebida);
      await expect(service.transicionar('os-1', S.Entregue)).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it('lança 404 quando a OS não existe', async () => {
      ...
      await expect(service.transicionar('inexistente', S.EmDiagnostico)).rejects.toThrow(
        NotFoundException,
      );
      expect(dataSource.queryRunner.rollbackTransaction).toHaveBeenCalled();
    });
